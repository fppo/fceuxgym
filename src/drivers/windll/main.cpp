...
int main2(int remainingFrames)
{
	while(GameInfo)
	{
		...
		if (remainingFrames > 0) {
			for (int i = 1; i < remainingFrames; i++) {
				FCEUI_Emulate_simple(&gfx, &sound, &ssize, 2);
			}
...
extern "C" {
	...
	__declspec(dllexport) const char* load_rom(const char* filename) {
		if (!filename) {
			ReloadRom();
		} else {
			if (!ALoad(filename)) {
	...
	__declspec(dllexport) int reset() {
		if (GameInfo) {
			FCEUI_ResetNES();
			return 0;
		}
	...
	__declspec(dllexport) uint32 save_state_to_memory(uint8* buf, uint32 bufsize) {
		if (!GameInfo) return 0;
		
		EMUFILE_MEMORY ms;
		if (FCEUSS_SaveMS(&ms, 0)) {
			uint32 size = ms.size();
			if (buf && bufsize >= size) {
				memcpy(buf, ms.buf(), size);
			}
			return size;
		}
		return 0;
	...
	__declspec(dllexport) int load_state_from_memory(const uint8* data, uint32 size) {
		if (!GameInfo || !data || size == 0) return -1;
		
		EMUFILE_MEMORY ms(const_cast<uint8*>(data), size);
	...
	__declspec(dllexport) uint32 save_state_lz4(uint8* buf, uint32 bufsize, int compressionLevel) {
		if (!GameInfo) return 0;
		
		EMUFILE_MEMORY ms;
		if (FCEUSS_SaveMS_LZ4(&ms, compressionLevel)) {
			uint32 size = ms.size();
			if (buf && bufsize >= size) {
				memcpy(buf, ms.buf(), size);
			}
	...
	__declspec(dllexport) int load_state_lz4(const uint8* data, uint32 size) {
		if (!GameInfo || !data || size == 0) return -1;
		EMUFILE_MEMORY ms(const_cast<uint8*>(data), size);
		if (FCEUSS_LoadFP_LZ4(&ms, SSLOADPARAM_NOBACKUP)) {
			return 0;
		}
		return -1;
	}
	...
	__declspec(dllexport) int load_config(const char* filename) {
